...
from mtrl.agent.components.classifier import Classifier
...
class Agent(AbstractAgent):
    ...
    def __init__(
        ...
        self.recurrent = self.actor.recurrent
    ...
    def get_initial_states(self):
        h_0, c_0 = None, None
        # TODO: hidden size dim
        if self.recurrent:
            h_0 = torch.zeros((
                self.actor.encoder.lstm.num_layers,
                self.num_envs,
                self.actor.encoder.lstm.hidden_size),
                dtype=torch.float)
    ...
    def act(
        ...
        with torch.no_grad():
            ...
            mtobs = MTObs(env_obs=obs, task_obs=env_index, task_info=task_info, hidden = hidden)
            mu, pi, _, _, next_hidden = self.actor(mtobs=mtobs)
            if sample:
                action = pi
            else:
                action = mu
            ...
            return action.detach().cpu().numpy(), next_hidden
    ...
    def get_Q(
        ...
        with torch.no_grad():
            ...
            if len(obs.shape) == 1 or len(obs.shape) == 3:
                obs = obs.unsqueeze(0)  # Make a batch
            mtobs = MTObs(env_obs=obs, task_obs=env_index, task_info=task_info, hidden = hidden)
            actor_Q1, actor_Q2, _, _, _ = self.critic(mtobs=mtobs, action=action, detach_encoder=True)
            actor_Q = torch.min(actor_Q1, actor_Q2)
            return actor_Q.detach().cpu().numpy()
    ...
    def _get_target_V(
        ...
        """
        mtobs = MTObs(env_obs=batch.next_env_obs, task_obs=None, task_info=task_info, hidden=batch.next_hidden)
        _, policy_action, log_pi, _, _= self.actor(mtobs=mtobs)
        target_Q1, target_Q2, _, _, ori_next_encoding = self.critic_target(mtobs=mtobs, action=policy_action)
        return (
    ...
    def update_critic(
        ...
        with torch.no_grad():
            target_V, ori_next_encoding = self._get_target_V(batch=batch, task_info=task_info)
            target_Q = reward + (batch.not_done * self.discount * target_V)


        # get current Q estimates
        mtobs = MTObs(env_obs=batch.env_obs, task_obs=None, task_info=task_info, hidden=batch.hidden)
        current_Q1, current_Q2, x, y, ori_encoding = self.critic(
            mtobs=mtobs,
            action=batch.action,
    ...
    def update_actor_and_alpha(
        ...
        _, pi, log_pi, log_std, _= self.actor(mtobs=mtobs, detach_encoder=True)
        actor_Q1, actor_Q2, _, _, _ = self.critic(mtobs=mtobs, action=pi, detach_encoder=True)       
        actor_Q = torch.min(actor_Q1, actor_Q2)

        # batch_env_obs_mean = self.obs_mean[task_info.env_index.squeeze(1)]
        # mtobs_mean = MTObs(
        #     env_obs=batch_env_obs_mean,
        #     task_obs=None,
        #     task_info=task_info,
        # )
        # actor_Q1_mean, actor_Q2_mean, _, _ = self.critic(mtobs=mtobs_mean, action=pi, detach_encoder=True)       
        # actor_Q_mean = torch.min(actor_Q1_mean, actor_Q2_mean)
        # pdb.set_trace()
        
        # actor_Q = torch.zeros_like(actor_Q)

        if self.loss_reduction == "mean":
            actor_loss = (self.get_alpha(batch.task_obs).detach() * log_pi - actor_Q).mean()
        ...
        #     logger.log(f"train/alpha_value_{id}", self.log_alpha[id].exp(), step)
        self._compute_gradient(
            loss=alpha_loss,
    ...
    def update_classifier(
        ...
        labels = batch.task_obs.squeeze(1)

        outputs = self.classifier(env_obs)

        ce_loss = loss_function(outputs, labels)
        # pdb.set_trace()
        logger.log("train/CrossEntropy_Loss", ce_loss, step)
