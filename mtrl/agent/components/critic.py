...
from mtrl.agent.components.actor import (
    ...
    check_if_should_use_attention_multi_head_policy,
)
...
class Critic(base_component.Component):
    ...
    def _make_qfunction(
        ...
        if self.should_concatenate_hidden_with_encoder and encoder_cfg.type in ["moe"]:
            if encoder_cfg.task_id_to_encoder_id_cfg.mode == "rnn_attention":
                obs_dim += encoder_cfg.task_id_to_encoder_id_cfg.rnn_attention.rnn_hidden_dim    
    ...
    def forward(
        ...
        if self.should_condition_encoder_on_task_info:
            encoding, next_hidden, ori_encoding = self.encode(mtobs=mtobs, detach=detach_encoder)
        else:
            ...
            encoding, next_hidden, ori_encoding = self.encode(mtobs=temp_mtobs, detach=detach_encoder)
        ...
        if self.should_use_multi_head_policy:
            if self.should_use_attention_multi_head_policy:
                q_mask = self.head_selection(task_info=task_info,env_obs=obs)
            else:    
        ...
        return q1, q2, None, None, ori_encoding
